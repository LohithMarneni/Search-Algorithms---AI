...
void iterativeDeepeningDFS(int n, int adj[max_nodes][max_nodes], int src, int goal, int max_depth) {
    // Perform iterative deepening for depth limits from 0 to max_depth
    for (int depth = 0; depth <= max_depth; depth++) {
        cout << "Depth Limit: " << depth << endl;
        dfsWithCost(n, adj, src, goal, depth);
        cout << endl;
    }
}
...
int main() {
    ...
    int max_depth = 3; // Example maximum depth for IDDFS
    ...
    iterativeDeepeningDFS(n, adj, src, goal, max_depth);
    return 0;
}