...
void dfsWithCost(int n, int adj[max_nodes][max_nodes], int src, int goal) {
    ...
    while (top >= 0) {
        ...
        for (int ne = 0; ne < n; ne++) {
            if (adj[node][ne] > 0 && !vis[ne]) {  // If there's an edge and the neighbor isn't visited
